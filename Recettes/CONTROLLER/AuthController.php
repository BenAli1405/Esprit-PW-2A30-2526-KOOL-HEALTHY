...
class AuthController
{
    public function inscrire($utilisateur, $profilNutritif)
    {
        ...
        try {
            ...
            $sql = "INSERT INTO utilisateurs (nom, email, mot_de_passe, role, poids, taille, imc, objectif, created_at)
                    VALUES (:nom, :email, :mot_de_passe, :role, :poids, :taille, :imc, :objectif, :created_at)";
            $req = $db->prepare($sql);
            $req->execute([
    ...
    public function connecter($nom, $mot_de_passe)
    {
        $db = config::getConnexion();
        try {
...
if (basename(__FILE__) === basename($_SERVER['SCRIPT_FILENAME'] ?? '')) {
    ...
    if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($controller->connecter($nom, $mot_de_passe)) {
